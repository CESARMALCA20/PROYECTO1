...
st.markdown("""
    ...
    div[data-baseweb="popover"] > div {
        ...
        border: 1px solid #4a5578 !important;
    }
    span[data-baseweb="tag"] {
        background-color: #1369CF !important;
    ...
    li[role="option"]:hover {
        background-color: #052BE5 !important;
        color: white !important;
    }
...
.stApp { background-color: #0A132E !important; font-family: 'DM Sans', sans-serif !important; }
#MainMenu, footer, header { visibility: hidden; }
.block-container { padding: 0 2rem 2rem 2rem !important; max-width: 100% !important; }
section[data-testid="stSidebar"] { display: none; }

.topbar {
    background: linear-gradient(135deg, #0d1524 0%, #0a1020 100%);
...
.logo-mark {
    ...
    color: white; box-shadow: 0 0 20px rgba(0, 195, 255, 0.35); letter-spacing: -0.5px;
}
.topbar-title { font-family: 'Syne', sans-serif; font-weight: 700; font-size: 15px; color: #f0f4ff; letter-spacing: -0.3px; }
.topbar-sub { font-size: 11px; color: #ffffff; font-weight: 400; margin-top: 1px; }
.topbar-badge {
    background: rgba(0, 195, 255, 0.08); border: 1px solid rgba(0, 195, 255, 0.2);
    border-radius: 20px; padding: 5px 14px; font-size: 11px; color: #00c3ff;
...
.section-label::after { content: ''; flex: 1; height: 1px; background: linear-gradient(90deg, rgba(74,85,120,0.4), transparent); }

.stMultiSelect label p, .stSelectbox label p {
    color: #B1BEF0!important; font-weight: 600 !important; font-size: 12px !important;
    text-transform: uppercase !important; letter-spacing: 1px !important; font-family: 'Syne', sans-serif !important;
}
.stDateInput > div > div > input {
    background-color: #0d1524 !important; border: 1px solid #1e2a45 !important;
    border-radius: 10px !important; color: #c8d0e8 !important;
    font-family: 'DM Sans', sans-serif !important; font-size: 13px !important; padding: 10px 14px !important;
}
div[data-baseweb="calendar"] { background-color: #0d1524 !important; border: 1px solid #2a3560 !important; border-radius: 12px !important; }
div[data-baseweb="calendar"] * { color: #789596 !important; }
div[data-baseweb="calendar"] [aria-selected="true"] > div { background-color: #6C63C7 !important; border-radius: 50% !important; }
div[data-baseweb="calendar"] button:hover > div { background-color: rgba(108,99,199,0.3) !important; border-radius: 50% !important; }
div[data-baseweb="select"] > div {
    background-color: #0d1524 !important; border: 1px solid #1e2a45 !important;
    border-radius: 10px !important; transition: border-color 0.2s !important;
}
div[data-baseweb="select"] > div:hover { border-color: rgba(0, 195, 255, 0.35) !important; }
div[data-baseweb="select"] > div:focus-within { border-color: rgba(0, 195, 255, 0.6) !important; box-shadow: 0 0 0 3px rgba(0, 195, 255, 0.1) !important; }
div[data-baseweb="popover"], div[data-baseweb="menu"] {
    background-color: #0d1524 !important; border: 1px solid #1e2a45 !important;
    border-radius: 10px !important; box-shadow: 0 20px 60px rgba(0,0,0,0.5) !important;
}
div[data-baseweb="menu"] li { background-color: #0d1524 !important; color: #c8d0e8 !important; font-size: 13px !important; }
div[data-baseweb="menu"] li:hover { background-color: #1a2540 !important; color: #00c3ff !important; }
div[data-baseweb="select"] span, div[data-baseweb="select"] input, div[data-baseweb="select"] div { color: #c8d0e8 !important; font-family: 'DM Sans', sans-serif !important; }

.kpi-card {
    color: #B1BEF0!important;
    background: linear-gradient(145deg, #0d1524, #09264A);
    border: 1px solid #1a2540; border-radius: 14px;
    padding: 20px 20px 18px 20px; position: relative; overflow: hidden;
    transition: transform 0.2s, border-color 0.2s, box-shadow 0.2s;
}
.kpi-card:hover { transform: translateY(-2px); border-color: var(--kpi-color, #1a2540); box-shadow: 0 8px 32px rgba(0,0,0,0.3), 0 0 0 1px var(--kpi-color, transparent); }
.kpi-card::before { content: ''; position: absolute; top: 0; left: 0; right: 0; height: 2px; background: linear-gradient(90deg, var(--kpi-color, transparent), transparent); opacity: 0.8; }
.kpi-card::after { content: ''; position: absolute; bottom: -30px; right: -30px; width: 100px; height: 100px; border-radius: 50%; background: radial-gradient(circle, var(--kpi-color, transparent) 0%, transparent 70%); opacity: 0.06; }
.kpi-header { display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 16px; }
.kpi-icon { width: 36px; height: 36px; border-radius: 9px; display: flex; align-items: center; justify-content: center; font-size: 17px; background: color-mix(in srgb, var(--kpi-color, #1a2540) 15%, transparent); }
.kpi-dot { width: 7px; height: 7px; border-radius: 50%; background: var(--kpi-color, #1a2540); box-shadow: 0 0 8px var(--kpi-color, transparent); margin-top: 4px; }
.kpi-value { font-family: 'Syne', sans-serif; font-size: 32px; font-weight: 800; color: #f0f4ff; line-height: 1; letter-spacing: -1px; margin-bottom: 6px; }
.kpi-label { font-size: 11px; font-weight: 600; color: #B1BEF0; text-transform: uppercase; letter-spacing: 1.5px; font-family: 'Syne', sans-serif; }

/* TABLA */
.table-scroll {
    max-height: 320px; overflow-y: auto; overflow-x: hidden;
    border-radius: 12px; box-shadow: 0 4px 24px rgba(0,0,0,0.22);
    border: 1px solid #e2e4f0; width: 100%;
}
.table-scroll::-webkit-scrollbar { width: 5px; }
.table-scroll::-webkit-scrollbar-track { background: #f3f4fb; }
.table-scroll::-webkit-scrollbar-thumb { background: #b0a8e8; border-radius: 3px; }
.custom-table { width: 100%; border-collapse: collapse; font-family: 'DM Sans', sans-serif; font-size: 13px; table-layout: fixed; }
...
.custom-table thead tr th {
    background-color: #1037C4; color: #ffffff; font-family: 'Syne', sans-serif;
    ...
    text-overflow: ellipsis; box-shadow: 0 2px 6px rgba(0,0,0,0.18);
}
...
.custom-table tbody tr:nth-child(even) td { background-color: #f3f4fb; }
.custom-table tbody tr td { color: #2d3058; padding: 10px 14px; border-bottom: 1px solid #e8eaf4; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
.custom-table tbody tr td.idx-col { overflow: visible !important; text-overflow: clip !important; padding: 10px 6px; }
.custom-table tbody tr:last-child td { background-color: #ede9ff !important; font-weight: 700; color: #4a3fa0; border-top: 2px solid #c4bef0; }
.custom-table tbody tr:hover td { background-color: #dde3f8 !important; color: #1a1e40; }
.custom-table td.num-col, .custom-table th.th-num { text-align: right; padding-right: 18px; font-variant-numeric: tabular-nums; }
.custom-table td.num-col { color: #6C63C7; font-weight: 700; font-family: 'Syne', sans-serif; }
.custom-table td.idx-col { color: #6a6f92; font-size: 12px; font-weight: 600; text-align: center; width: 48px; min-width: 48px; overflow: visible; white-space: nowrap; }
.custom-table td.fecha-col { font-size: 12px; color: #6a6f92; letter-spacing: 0.2px; }
...
.nav-btn {
    ...
    color: #00c3ff; font-family: 'Syne', sans-serif; letter-spacing: 0.5px;
    cursor: pointer; display: flex; align-items: center; gap: 7px;
    transition: background 0.2s, border-color 0.2s;
    white-space: nowrap;
}
.nav-btn:hover { background: rgba(0,195,255,0.15); border-color: rgba(0,195,255,0.4); }
.nav-btn .arrow { font-size: 9px; transition: transform 0.2s; }
.nav-btn.open .arrow { transform: rotate(180deg); }
.nav-btn.open { background: rgba(0,195,255,0.15); border-color: rgba(0,195,255,0.4); }
.nav-dropdown-menu {
    display: none; position: absolute; top: calc(100% + 4px); right: 0;
    background: linear-gradient(145deg, #0d1524, #0a1020);
    border: 1px solid rgba(0,195,255,0.15); border-radius: 14px;
    min-width: 220px; padding: 8px 0; z-index: 10000;
    box-shadow: 0 20px 60px rgba(0,0,0,0.6), 0 0 0 1px rgba(0,195,255,0.05);
    backdrop-filter: blur(16px);
}
.nav-dropdown-menu.open { display: block; }
.nav-dropdown-header {
    font-family: 'Syne', sans-serif; font-size: 10px; font-weight: 700;
    color: #4a5578; letter-spacing: 2px; text-transform: uppercase;
    padding: 8px 18px 6px 18px; border-bottom: 1px solid rgba(74,85,120,0.3);
    margin-bottom: 4px;
}
.nav-dropdown-item {
    display: flex; align-items: center; gap: 10px;
    padding: 9px 18px; font-size: 13px; color: #c8d0e8;
    font-family: 'DM Sans', sans-serif; cursor: pointer;
    transition: background 0.15s, color 0.15s;
    text-decoration: none; border: none; background: none; width: 100%; text-align: left;
}
.nav-dropdown-item:hover { background: rgba(0,195,255,0.08); color: #00c3ff; }
.nav-dropdown-item.active { color: #00c3ff; background: rgba(0,195,255,0.06); }
.nav-dropdown-item .item-num {
    width: 20px; height: 20px; border-radius: 6px; display: flex; align-items: center;
    justify-content: center; font-size: 10px; font-weight: 800; font-family: 'Syne', sans-serif;
    background: rgba(0,195,255,0.1); color: #00c3ff; flex-shrink: 0;
}
.nav-dropdown-item.active .item-num { background: rgba(0,195,255,0.2); }
.nav-item-label { color: inherit; }
.nav-item-badge {
    margin-left: auto; font-size: 9px; background: rgba(0,195,255,0.12);
    border: 1px solid rgba(0,195,255,0.2); border-radius: 8px;
    padding: 2px 7px; color: #00c3ff; font-weight: 600; white-space: nowrap;
}
.nav-item-badge.soon {
    background: rgba(74,85,120,0.2); border-color: rgba(74,85,120,0.3);
    color: #4a5578;
}
</style>
        ...
        <div class="nav-menu">
            <div class="nav-dropdown">
                ...
                <div class="nav-dropdown-menu" id="nav-dropdown-menu">
                    ...
                    <a href="?page=prenatal" target="_self" style="display:flex;align-items:center;gap:10px;padding:9px 18px;font-size:13px;color:#00c3ff;text-decoration:none;background:rgba(0,195,255,0.06);">
                        <span style="width:20px;height:20px;border-radius:6px;display:inline-flex;align-items:center;justify-content:center;font-size:10px;font-weight:800;background:rgba(0,195,255,0.2);color:#00c3ff;">1</span>
                        &nbsp;Periodo Prenatal&nbsp;<span style="margin-left:auto;font-size:9px;background:rgba(0,195,255,0.12);border:1px solid rgba(0,195,255,0.2);border-radius:8px;padding:2px 7px;color:#00c3ff;font-weight:600;">Activo</span>
                    </a>
                    <a href="?page=nino" target="_self" style="display:flex;align-items:center;gap:10px;padding:9px 18px;font-size:13px;color:#00c3ff;text-decoration:none;background:rgba(0,195,255,0.06);">
                        <span style="width:20px;height:20px;border-radius:6px;display:inline-flex;align-items:center;justify-content:center;font-size:10px;font-weight:800;background:rgba(0,195,255,0.2);color:#00c3ff;">2</span>
                        &nbsp;Niño&nbsp;<span style="margin-left:auto;font-size:9px;background:rgba(0,195,255,0.12);border:1px solid rgba(0,195,255,0.2);border-radius:8px;padding:2px 7px;color:#00c3ff;font-weight:600;">Activo</span>
                    </a>
                    <a href="?page=adolescente" target="_self" style="display:flex;align-items:center;gap:10px;padding:9px 18px;font-size:13px;color:#00c3ff;text-decoration:none;background:rgba(0,195,255,0.06);">
                        <span style="width:20px;height:20px;border-radius:6px;display:inline-flex;align-items:center;justify-content:center;font-size:10px;font-weight:800;background:rgba(0,195,255,0.2);color:#00c3ff;">3</span>
                        &nbsp;Adolescente&nbsp;<span style="margin-left:auto;font-size:9px;background:rgba(0,195,255,0.12);border:1px solid rgba(0,195,255,0.2);border-radius:8px;padding:2px 7px;color:#00c3ff;font-weight:600;">Activo</span>
                    </a>
                    <a href="?page=joven" target="_self" style="display:flex;align-items:center;gap:10px;padding:9px 18px;font-size:13px;color:#00c3ff;text-decoration:none;background:rgba(0,195,255,0.06);">
                        <span style="width:20px;height:20px;border-radius:6px;display:inline-flex;align-items:center;justify-content:center;font-size:10px;font-weight:800;background:rgba(0,195,255,0.2);color:#00c3ff;">4</span>
                        &nbsp;Joven&nbsp;<span style="margin-left:auto;font-size:9px;background:rgba(0,195,255,0.12);border:1px solid rgba(0,195,255,0.2);border-radius:8px;padding:2px 7px;color:#00c3ff;font-weight:600;">Activo</span>
                    </a>
                    <a href="?page=adulto" target="_self" style="display:flex;align-items:center;gap:10px;padding:9px 18px;font-size:13px;color:#00c3ff;text-decoration:none;background:rgba(0,195,255,0.06);">
                        <span style="width:20px;height:20px;border-radius:6px;display:inline-flex;align-items:center;justify-content:center;font-size:10px;font-weight:800;background:rgba(0,195,255,0.2);color:#00c3ff;">5</span>
                        &nbsp;Adulto&nbsp;<span style="margin-left:auto;font-size:9px;background:rgba(0,195,255,0.12);border:1px solid rgba(0,195,255,0.2);border-radius:8px;padding:2px 7px;color:#00c3ff;font-weight:600;">Activo</span>
                    </a>
                    <a href="?page=adulto_mayor" target="_self" style="display:flex;align-items:center;gap:10px;padding:9px 18px;font-size:13px;color:#00c3ff;text-decoration:none;background:rgba(0,195,255,0.06);">
                        <span style="width:20px;height:20px;border-radius:6px;display:inline-flex;align-items:center;justify-content:center;font-size:10px;font-weight:800;background:rgba(0,195,255,0.2);color:#00c3ff;">6</span>
                        &nbsp;Adulto_Mayor&nbsp;<span style="margin-left:auto;font-size:9px;background:rgba(0,195,255,0.12);border:1px solid rgba(0,195,255,0.2);border-radius:8px;padding:2px 7px;color:#00c3ff;font-weight:600;">Activo</span>
                    </a>
                </div>
            </div>
        </div>
...
# ─── JS MENÚ (click para abrir/cerrar) ───────────────────────────────────────
...
# - 1 mes seleccionado    → activo, anclado a ese mes
...
with cal1:
    st.markdown(f'''<p style="font-family:Syne,sans-serif;font-size:11px;font-weight:700;
        color:{"#B1BEF0" if calendario_activo else "#4a5578"};
    ...
    if calendario_activo:
        ...
        st.markdown('''<div style="background:#0a1020;border:1px solid #1e2a45;border-radius:10px;
            padding:10px 14px;font-family:DM Sans,sans-serif;font-size:13px;
            color:#2a3555;cursor:not-allowed;user-select:none;">DD/MM/YYYY</div>''',
            unsafe_allow_html=True)
        fecha_desde = None
...
with cal3:
    ...
    if msg_cal is not None:
        ...
        st.markdown(f'''
        <div style="background:rgba(30,42,69,0.5);border:1px dashed {borde};
            border-radius:10px;padding:11px 18px;font-family:DM Sans,sans-serif;
        ...
        st.markdown(f'''
        <div style="background:rgba(108,99,199,0.15);border:1px solid rgba(108,99,199,0.4);
            border-radius:10px;padding:11px 18px;font-family:DM Sans,sans-serif;
            font-size:13px;color:#FFFFFF;display:flex;align-items:center;gap:10px;">
            <span style="font-size:18px;">🗓️</span>
            <span>Filtrando del&nbsp;
                <b style="color:#a89ef0;font-size:14px;">{d_str}</b>
                &nbsp;al&nbsp;
                <b style="color:#a89ef0;font-size:14px;">{h_str}</b>
            </span>
        </div>''', unsafe_allow_html=True)
...
# ─── 2. FILTROS DE CATEGORÍA (IPRESS, ITEM, ETC) ───────────────────────────
if ipress_sel: 
...
def style_radar(fig, height=300):
    fig.update_layout(
        ...
        font=dict(family="DM Sans, sans-serif", color=FONT_COLOR, size=10),
        height=height,
        margin=dict(l=70, r=70, t=50, b=50),
        showlegend=False,
        hoverlabel=dict(
            bgcolor="#111e42", bordercolor="#4a5578",
            font=dict(color="#f0f4ff", family="DM Sans", size=12)
        ),
        polar=dict(
            bgcolor="rgba(13,21,36,0.7)",
            domain=dict(x=[0.05, 0.95], y=[0.05, 0.95]),
            angularaxis=dict(
                tickfont=dict(size=9, color="#c8d0e8", family="DM Sans"),
                linecolor="#2a3560", gridcolor="#1e2a45", direction="clockwise",
            ),
            radialaxis=dict(
                visible=True,
                tickfont=dict(size=8, color="#4a5578"),
                gridcolor="#253560", linecolor="#253560",
...
def style_bar(fig, height=300):
    ...
    fig.update_layout(
        ...
        xaxis=dict(
            showgrid=True, gridcolor="#1e2a45", zeroline=False,
            tickfont=dict(color="#4a5578", size=9),
            linecolor="#1e2a45"
        ),
        yaxis=dict(
            showgrid=False, zeroline=False,
            tickfont=dict(color="#c8d0e8", size=10, family="DM Sans"),
            linecolor="#1e2a45",
            automargin=True,
        )
    )
...
def hacer_barras(etiquetas_raw, vals, color_base, degradado_colors):
    ...
    fig.add_trace(go.Bar(
        ...
        textfont=dict(color="#c8d0e8", size=10, family="Syne"),
...
CARD_OPEN  = """<div style="
    background: linear-gradient(160deg, #111827 0%, #153769 100%);
    border: 1px solid #2a3560; border-radius: 14px;
    padding: 14px 12px 4px 12px;
    box-shadow: 0 4px 20px rgba(0,0,0,0.35), inset 0 1px 0 rgba(255,255,255,0.04);
    margin-bottom: 0px;">"""
CARD_CLOSE = "</div>"

def chart_label(titulo, color):
    return f"""<div style="font-family:'Syne',sans-serif;font-size:11px;font-weight:700;
        color:#FFFFFF;text-transform:uppercase;letter-spacing:1.5px;margin-bottom:4px;
        display:flex;align-items:center;gap:8px;">
        <span style="display:inline-block;width:3px;height:13px;background:{color};
            border-radius:2px;flex-shrink:0;"></span>{titulo}</div>"""

# ─── 8. GRÁFICOS ──────────────────────────────────────────────────────────────
st.markdown('<div class="section-label">Análisis Operativo</div>', unsafe_allow_html=True)
c1, c2, c3, c4 = st.columns(4)

# ── Chart 1: Top Actividades — Radar (muchas categorías, funciona bien) ──
with c1:
    st.markdown(CARD_OPEN + chart_label("Top Actividades", "#06d6a0"), unsafe_allow_html=True)
    res_it = (
    ...
    if not res_it.empty:
        fig1 = hacer_radar(
            ...
            res_it["Cant"].tolist(),
            '#06d6a0', 'rgba(6,214,160,0.20)', "Actividades"
        )
        st.plotly_chart(style_radar(fig1), use_container_width=True)
    st.markdown(CARD_CLOSE, unsafe_allow_html=True)

# ── Chart 2: Diagnósticos — Barras horizontales ──
with c2:
    st.markdown(CARD_OPEN + chart_label("Diagnósticos", "#00c3ff"), unsafe_allow_html=True)
    res_dg = (
    ...
    if not res_dg.empty:
        # Reemplazar códigos de diagnóstico por nombres legibles
        ...
        degradado_cyan = ["#00c3ff","#00aee6","#009ecc","#008db3","#007d9a","#006c80"]
        fig2 = hacer_barras(
            res_dg["Tipo_Diagnostico"].tolist(),
            res_dg["Cant"].tolist(),
            '#00c3ff', degradado_cyan
        )
        st.plotly_chart(style_bar(fig2), use_container_width=True)
    st.markdown(CARD_CLOSE, unsafe_allow_html=True)

# ── Chart 3: Atenciones x IPRESS — Barras horizontales ──
with c3:
    st.markdown(CARD_OPEN + chart_label("Atenciones × IPRESS", "#7b61ff"), unsafe_allow_html=True)
    res_ip = (
    ...
    if not res_ip.empty:
        degradado_purple = ["#7b61ff","#6e55e8","#6149d1","#543dba","#4831a3","#3b268c"]
        fig3 = hacer_barras(
            res_ip["Nombre_Establecimiento"].tolist(),
            res_ip["Cant"].tolist(),
            '#7b61ff', degradado_purple
        )
        st.plotly_chart(style_bar(fig3), use_container_width=True)
    st.markdown(CARD_CLOSE, unsafe_allow_html=True)

# ── Chart 4: Tipo de Seguro — Barras horizontales ──
with c4:
    st.markdown(CARD_OPEN + chart_label("Tipo de Seguro", "#ffd166"), unsafe_allow_html=True)
    res_sg = (
    ...
    # Quitar filas con valor "None" string
    res_sg = res_sg[res_sg["Descripcion_Financiador"].astype(str).str.strip() != "None"]
    if not res_sg.empty:
        degradado_gold = ["#ffd166","#f0be55","#e0ab44","#d09833","#c08522","#b07211"]
        fig4 = hacer_barras(
            res_sg["Descripcion_Financiador"].tolist(),
            res_sg["Cant"].tolist(),
            '#ffd166', degradado_gold
        )
        st.plotly_chart(style_bar(fig4), use_container_width=True)
    st.markdown(CARD_CLOSE, unsafe_allow_html=True)
...
if personal_elegido != "— Todos —" and not res_p_full.empty:
    ...
    <div style="display:inline-flex;align-items:center;gap:10px;
        background:rgba(108,99,199,0.15);border:1px solid rgba(108,99,199,0.5);
        border-radius:20px;padding:6px 16px;margin-bottom:4px;
        font-family:'DM Sans',sans-serif;font-size:13px;color:#c8d0e8;">
        <span style="font-size:16px;">👤</span>
        Mostrando pacientes de: <b style="color:#a89ef0;">{personal_elegido}</b>
    </div>""", unsafe_allow_html=True)
...
with t1:
    st.markdown("""<div style="font-family:'Syne',sans-serif;font-size:12px;font-weight:700;color:#B1BEF0;text-transform:uppercase;letter-spacing:1.5px;margin-bottom:10px;">
        <span style="display:inline-block;width:3px;height:13px;background:#00c3ff;border-radius:2px;margin-right:8px;vertical-align:middle;"></span>
...
with t2:
    ...
    st.markdown(f"""<div style="font-family:'Syne',sans-serif;font-size:12px;font-weight:700;color:#B1BEF0;text-transform:uppercase;letter-spacing:1.5px;margin-bottom:10px;display:flex;align-items:center;gap:10px;">
        <span style="display:inline-block;width:3px;height:13px;background:#7b61ff;border-radius:2px;"></span>
        {titulo_pac}
        <span style="font-size:11px;font-weight:700;color:#FFFFFF;background:rgba(108,99,199,0.12);
            border:1px solid rgba(108,99,199,0.3);border-radius:20px;padding:1px 10px;
...
# ─── Footer ───────────────────────────────────────────────────────────────────
st.markdown("""
<div style="margin-top:40px;padding:20px 0;border-top:1px solid #1a2540;
    text-align:center;font-size:11px;color:#ffffff;font-family:'DM Sans',sans-serif;">
